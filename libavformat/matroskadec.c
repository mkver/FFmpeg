    ...
    for (i = 0; i < matroska->tracks.nb_elem; i++) {
        ...
        if (!strcmp(track->codec_id, "V_MS/VFW/FOURCC") &&
            ...
            if (!extradata)
                return AVERROR(ENOMEM);
            write_ptr = extradata;
            bytestream_put_be32(&write_ptr, AV_RB32("TTA1"));
            bytestream_put_le16(&write_ptr, 1);
            bytestream_put_le16(&write_ptr, track->audio.channels);
            bytestream_put_le16(&write_ptr, track->audio.bitdepth);
            bytestream_put_le32(&write_ptr, track->audio.out_samplerate);
            bytestream_put_le32(&write_ptr, av_rescale((matroska->duration * matroska->time_scale),
