                                  ...
                                  int native_id, int qt_id)
{
    ...
    if (codecpriv_size) {
        ...
        if (!*size || sizediff == 0 || sizediff > 1) {
            ...
            av_log(s, AV_LOG_ERROR, "Can't update CodecPrivate as not enough "
                   "size has been reserved for it.\n");
...
//                                                 st->time_base);
    ...
    if (mkv->mode != MODE_WEBM || par->codec_id != AV_CODEC_ID_WEBVTT) {
        ...
        ret = mkv_write_codecprivate(s, pb, par, &mkv->tracks[i].codecpriv_size,
                                     native_id, qt_id);
        if (ret < 0)
            return ret;
    }
...
static int mkv_check_new_extra_data(AVFormatContext *s, AVPacket *pkt)
{
    ...
    case AV_CODEC_ID_AAC:
        if (side_data_size && (s->pb->seekable & AVIO_SEEKABLE_NORMAL) && !mkv->is_live) {
            ...
            mkv_write_codecprivate(s, mkv->tracks_bc, par, &(int) { 0 }, 1, 0);
    ...
    case AV_CODEC_ID_H264:
        if (side_data_size && (s->pb->seekable & AVIO_SEEKABLE_NORMAL) && !mkv->is_live) {
            ...
            codecpriv_par->extradata_size = side_data_size;
            memset(codecpriv_par->extradata + side_data_size, 0,
                   AV_INPUT_BUFFER_PADDING_SIZE);
            curpos = avio_tell(mkv->tracks_bc);
            avio_seek(mkv->tracks_bc, track->codecpriv_offset, SEEK_SET);
            mkv_write_codecprivate(s, mkv->tracks_bc, codecpriv_par,
