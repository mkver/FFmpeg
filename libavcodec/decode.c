...
void ff_thread_progress_unref(ProgressFrame *f)
{
...
void ff_thread_progress_replace(ProgressFrame *dst, const ProgressFrame *src)
{
    if (dst == src)
        return;
    ff_thread_progress_unref(dst);
...
int ff_decode_preinit(AVCodecContext *avctx)
{
    ...
    if (ffcodec(avctx->codec)->caps_internal & FF_CODEC_CAP_USES_PROGRESSFRAMES) {
        avci->progress_frame_pool =
            ff_refstruct_pool_alloc_ext(sizeof(ProgressInternal), 0, NULL,
