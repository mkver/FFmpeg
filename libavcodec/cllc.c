                          ...
                          VLC *vlc, uint8_t *outbuf)
{
    ...
    for (i = 0; i < ctx->avctx->width; i++) {
        ...
        code = get_vlc(gb, vlc[0].table, VLC_BITS, VLC_BITS * VLC_DEPTH);
        ...
        if (dst[0]) {
            /* Red */
            code = get_vlc(gb, vlc[1].table, VLC_BITS, VLC_BITS * VLC_DEPTH);

            pred[1] += code;
            dst[1]   = pred[1];

            /* Green */
            code = get_vlc(gb, vlc[2].table, VLC_BITS, VLC_BITS * VLC_DEPTH);

            pred[2] += code;
            dst[2]   = pred[2];

            /* Blue */
            code = get_vlc(gb, vlc[3].table, VLC_BITS, VLC_BITS * VLC_DEPTH);
                                     ...
                                     int *top_left, VLC *vlc, uint8_t *outbuf)
{
    ...
    for (i = 0; i < ctx->avctx->width; i++) {
        pred  += get_vlc(gb, vlc->table, VLC_BITS, VLC_BITS * VLC_DEPTH);
        dst[0] = pred;
        dst   += 3;
    }
