...
static void vp3_decode_flush(AVCodecContext *avctx)
{
    ...
    ff_thread_progress_unref(&s->golden_frame);
    ff_thread_progress_unref(&s->last_frame);
    ff_thread_progress_unref(&s->current_frame);
}
...
static void update_frames(AVCodecContext *avctx)
{
    ...
    if (s->keyframe)
        ff_thread_progress_replace(&s->golden_frame, &s->current_frame);
...
{
    ff_thread_progress_replace(&dst->current_frame, &src->current_frame);
    ff_thread_progress_replace(&dst->golden_frame,  &src->golden_frame);
    ff_thread_progress_replace(&dst->last_frame,    &src->last_frame);
}
                            ...
                            int *got_frame, AVPacket *avpkt)
{
    ...
    if (s->keyframe) {
        ...
        if (!s->golden_frame.f) {
            ...
            ff_thread_progress_replace(&s->last_frame, &s->golden_frame);
