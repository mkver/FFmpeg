...
{
    ff_thread_progress_replace(&dst->tf, &src->tf);
...
static av_cold int vp9_decode_free(AVCodecContext *avctx)
{
    ...
    ff_refstruct_pool_uninit(&s->frame_extradata_pool);
    for (i = 0; i < 8; i++) {
        ff_thread_progress_unref(&s->s.refs[i]);
        ff_thread_progress_unref(&s->next_refs[i]);
    }
                            ...
                            int *got_frame, AVPacket *pkt)
{
    ...
    if ((ret = decode_frame_header(avctx, data, size, &ref)) < 0) {
        ...
        for (int i = 0; i < 8; i++)
            ff_thread_progress_replace(&s->next_refs[i], &s->s.refs[i]);
    ...
    // ref frame setup
    for (i = 0; i < 8; i++) {
        ff_thread_progress_replace(&s->next_refs[i],
...
finish:
    // ref frame setup
    for (int i = 0; i < 8; i++)
        ff_thread_progress_replace(&s->s.refs[i], &s->next_refs[i]);
...
static int vp9_decode_update_thread_context(AVCodecContext *dst, const AVCodecContext *src)
{
    ...
    for (int i = 0; i < 8; i++)
        ff_thread_progress_replace(&s->s.refs[i], &ssrc->next_refs[i]);
