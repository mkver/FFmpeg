                               ...
                               void *content_ref)
{
    ...
    if (content_ref) {
        av_assert1(content == content_ref);
...
static void cbs_default_free_unit_content(void *opaque, void *data)
{
    ...
    if (desc->content_type == CBS_CONTENT_TYPE_INTERNAL_REFS) {
        ...
        for (i = 0; i < desc->type.ref.nb_offsets; i++) {
            void **ptr = (void**)((char*)data + desc->type.ref.offsets[i]);
...
{
    return ff_refstruct_allocz_ext(desc->content_size, (void*)desc,
...
int ff_cbs_make_unit_writable(CodedBitstreamContext *ctx,
                              CodedBitstreamUnit *unit)
{
    FFRefStruct ref = unit->content_ref;
    int err;

    av_assert0(unit->content);
    if (ref && ff_refstruct_is_writable(ref))
        return 0;
