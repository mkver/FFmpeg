...
static int h264_metadata_update_extradata(AVBSFContext *bsf, AVPacket *out)
{
    ...
    for (int i = 0; i <= header->max_sps; i++) {
        if ((extra->sps_status[i] & (IS_IN_HEADER|HEADER_WAS_USED)) == IS_IN_HEADER) {
            ...
            err = ff_cbs_insert_unit_content(ctx->cbc_out, au, -1, H264_NAL_SPS,
                                             header->sps[i].ps,
                                             header->sps[i].ps_ref);
            if (err < 0)
                return err;
    ...
    for (int i = 0; i <= header->max_pps; i++) {
        if ((extra->pps_status[i] & (IS_IN_HEADER|HEADER_WAS_USED)) == IS_IN_HEADER) {
            ...
            err = ff_cbs_insert_unit_content(ctx->cbc_out, au, -1, H264_NAL_PPS,
                                             header->pps[i].ps,
                                             header->pps[i].ps_ref);
            if (err < 0)
                return err;
                                    ...
                                    CodedBitstreamUnit *unit)
{
    ...
    if(ctx->delete_filler & 2) {
        int err;
        err = ff_cbs_make_unit_writable(ctx->cbc, unit, &sps);
        if (err < 0)
            return err;
                                    ...
                                    CodedBitstreamUnit *unit)
{
    ...
    if (ctx->misc.misc.interlaced && sps->frame_mbs_only_flag &&
        ...
        err = ff_cbs_make_unit_writable(ctx->cbc, unit, &pps);
        if (err < 0)
            return err;
                                  ...
                                  CodedBitstreamUnitType type)
{
    ...
    for (int i = 0; i <= max_ps; i++) {
        if (status[i] & WRITE) {
            err = ff_cbs_insert_unit_content(ctx, au, pos++, type,
                                                   ps[i].ps, NULL);
            if (err < 0)
                return err;
...
{
    H264MetadataContext *ctx = bsf->priv_data;
    CodedBitstreamH264Context *out = ctx->cbc_out->priv_data;
    Extradata *extra = ctx->extra;
    Header   *header = ctx->header;
    ...
    if (write) {
        ...
        err = h264_metadata_write_ps(ctx->cbc_out, au, extra->max_sps, &position,
                                     extra->sps, extra->sps_status, H264_NAL_SPS);
        if (err < 0)
            goto fail;

        err = h264_metadata_write_ps(ctx->cbc_out, au, extra->max_pps, &position,
...
static int h264_metadata_filter(AVBSFContext *bsf, AVPacket *pkt)
{
    ...
    if (ctx->sei_user_data && (has_ps & 1 || !ctx->done_first_au)) {
            ...
            if (flags & (PROGRESSIVE|MOD_FRAME_NUM|MOD_POC)
                ...
                if (flags & (MOD_FRAME_NUM|MOD_POC) && !idr_slice) {
                    ...
                    if (!mmco && slice->adaptive_ref_pic_marking_mode_flag) {
                        for (j = 0; slice->mmco[j].memory_management_control_operation; j++) {
    ...
    if (ctx->dts) {
        if (pkt->dts != AV_NOPTS_VALUE) {
            ...
            if (pkt->pts != AV_NOPTS_VALUE && pkt->dts > pkt->pts)
                av_log(bsf, AV_LOG_WARNING, "dts %lli still bigger "
                       "than pts %lli.\n", pkt->dts, pkt->pts);
        }
    }
...
static int h264_metadata_init(AVBSFContext *bsf)
{
        ...
        if (ctx->qp.qp.state > 0) {
            if (!pps_unit) {
                ...
                for (i = 1; i <= ctx->qp.qp.num_qp_minus_1; i++) {
                    err = ff_cbs_insert_unit_content(ctx->cbc, au, -1,
                                                     H264_NAL_PPS,
                                                     pps_unit->content,
                    ...
                    err = ff_cbs_make_unit_writable(ctx->cbc, pps_unit, &pps);
                    if (err < 0)
                        goto fail;
    ...
    if (ctx->dts) {
        ctx->dts = -bsf->time_base_out.den * ctx->dts /
                       (bsf->time_base_out.num * 1000);
    }
